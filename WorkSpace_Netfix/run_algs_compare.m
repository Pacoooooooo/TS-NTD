...
%%
data_name = './DataTensors/netfix_sub_tensor_dense.mat';
disp(['[Info]正在加载数据:', data_name]);
tic;
load(data_name);
visit_tensor = dense_visit_tensor;
...
% 将数据归一化
FILL_NUM = 1 ./ (1 + exp(-(-1)));
NORM_METHOD = 'sigmoid'; % sigmoid | rescale
...
% 划分训练/测试集
train_data = genre_tensor(:,:,1:19);
test_data = genre_tensor(:,:,20);
preprocess_time = toc;
...
opts.beta = 0.5;
tsntd_opts = opts;
tsntd_opts.alpha_U = 0.8;
tsntd_opts.alpha_L = 0.8;
tsntd_opts.alpha_T = 0.8;
tic;
